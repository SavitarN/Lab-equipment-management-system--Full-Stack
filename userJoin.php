<?php
$errUsername=$errmail=$errpass=$errCpass=$erraddress=$errphone='';
$Username=$email=$password=$Cpassword=$address=$phone='';
$flag=true;
if(isset($_POST['signin'])){
    ...
    if(empty(trim($email))){
        $errmail="email field cannot be empty";
        $flag=false;
    }
    elseif(!filter_var($email,FILTER_VALIDATE_EMAIL)){
        $errmaiil="invalid mail";
        $flag=false;
    }
    else{
    ...
    if(empty(trim($address))){
        $erraddress="address field cannot be empty";
        $flag=false;
    }
    else{
    ...
    if(empty(trim($phone))){
        $errphone="phone field cannot be empty";
        $flag=false;
    }
    else{
        if(!preg_match($phc,$phone)){
            $errphone="Error in phone number";
            $flag=false;
        }
        ...
        $phone=test_input($phone);
                ...
                if($flag && $password==$Cpassword){
                    ...
                    $sql="INSERT INTO customer(customer_username,customer_email,customer_password,customer_address,customer_phone)VALUES('$Username','$email','$hash','$address','$phone')";
                    $result=mysqli_query($conn,$sql);
                    if($result){
                        echo"<script>alert('Joined succesfully')</script>";
                    }
                }
    ...
    <div class="container-fluid p-0">
        ...
        <nav class="navbar navbar-expand-lg bg-info">
            <div class="container-fluid">
                <img src="./images/logo.jpg" class="" height="400px" width="550px" style="border:10px solid blue">
                ...
                <div class="contactform">
                    ...
                    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
                        ...
                        <P class="fonts">Email</P><span class="error"><?php echo $errmail;?></span>
                        <input class="first" type="email" name="mail">
                        ...
                        <P class="fonts">Address</P><span class="error"><?php echo $erraddress;?></span>
                        <input class="first" type="text" name="address">



                        <P class="fonts">Phone</P><span class="error"><?php echo $errphone;?></span>
                        <input class="first" type="number" name="phone">

                        <span class="foot">
                            <input class="button" type="submit" value="Create Account" name="signin">
                            <span>
                                <p class="login">Have an account? <a href="Userlogin.php">Login</a></p>
